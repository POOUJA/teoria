...
public:
    ...
    Punto2D<T>& operator=(const Punto2D<T> &orig);
    bool igualA(const Punto2D<T>& p) const;
    bool operator==(const Punto2D<T>& p) const;
    float distanciaA(const Punto2D<T>& p) const;
...
template<typename T>
Punto2D<T>::Punto2D(T x, T y):_x(x),_y(y) {
}
...
template<typename T>
Punto2D<T>& Punto2D<T>::operator=(const Punto2D<T> &orig) {
    if (&orig!=this) {
        _x=orig._x;
        _y=orig._y;
    };
...
template<typename T>
bool Punto2D<T>::igualA(const Punto2D<T>& p) const {
    return _x==p._x && _y==p._y;
}

/**Comprueba si un dos puntos son iguales*/
template<typename T>
bool Punto2D<T>::operator==(const Punto2D<T>& p) const {
    return igualA(p);
}

/**Devuelve la distancia entre dos puntos*/
template<typename T>
float Punto2D<T>::distanciaA(const Punto2D<T>& p) const {
